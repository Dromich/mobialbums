<div id="system_message"></div>
<div id="collection-cover" style="background-image: url('[xfvalue_image_url_topimg]'); ">
  <div class="cover-detail">
    <div class="cover-detail-content">
      <h1 class="cover-collection-name">{title}</h1>
      <br>
      <h4 class="cover-collection-date"></h4>    
    </div>

    <div id="enter_button_block">
      <a id="enter_button" class="white_button" href="#">До альбому</a>
    </div>
  </div>

  <div class="cover-logo-wrapper">
    <a class="business-name-link" href="https://www.instagram.com/ksuha.romaniv/" > Ксюха Романів</a>
  </div>
  
</div>
<div id="nav-wrapper" class="visible-desktop ">
    <div id="nav" class="navbar affix-top">
        <div class="navbar-inner-desktop">
            <div class="nav-collapse in collapse" style="height: auto;">
                <div class="pull-left nav-business-name" align="left" style="float:left;">
                    <h1 id="album_title">{title}</h1>
                    <a id="business-link" href="https://www.instagram.com/ksuha.romaniv/" rel="noopener noreferrer">Ксюха Романів</a>
                    
                </div>

                <div style="float:right; padding-top:6px;">
                  <a onclick="expand_block('share');" class="black_button">ПОДІЛИТИСЬ</a>
                  <!--a onclick="expand_block('contacts');" class="black_button">ЗВ'ЯЗАТИСЬ</a-->
                </div>
     
                <br clear="all"/>
                
                          <div align="center" id="share_block" style="display: none;">
                    
                    <div ></div>
                    <div class="rightyash nav-expand">
                        <center><h3 class="share_text">Поділіться з друзями через:</h3></center>
						<script type="text/javascript" src="//yastatic.net/es5-shims/0.0.2/es5-shims.min.js" charset="utf-8"></script>
						<script type="text/javascript" src="//yastatic.net/share2/share.js" charset="utf-8"></script>
						<div class="ya-share2" data-services="facebook,vkontakte,odnoklassniki,twitter,viber,whatsapp,telegram"></div>
</div>                              
</div>
               
               
            </div>
        </div>
    </div>
</div>
[xfvalue_mobalbum]

<div  id="contacts_block" class="contact_infor">
	<hr>
	<div class="nav-expand"><i class="fa fa-fw fa-phone"></i> <a href="https://www.instagram.com/ksuha.romaniv/">Ксюха Романів</a></div>
<div class="nav-expand"><i class="fa fa-fw fa-phone"></i> <a href="tel:80971640304">80971640304</a></div>
<div class="nav-expand"><i class="fa fa-fw fa-at"></i> <a href="mailto:kumednaksuha@gmail.com">kumednaksuha@gmail.com</a></div>
						
</div>